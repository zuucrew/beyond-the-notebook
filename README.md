...
| `docs/ESTIMATE.md` | What it costs to build, in money and evenings |
| `src/` | The code |
...
design decision, because overbuilding wastes as much time as underbuilding.
...
Levels compose with deployment independently, so `L2 + deployed` means a container
...
- Diagrams are Mermaid in fenced code blocks. They are text, so they diff in review and
