...
- **Identity** — no auth; a reviewer is a string
...
is what makes that recoverable — the sweep in §7.8 can find them.
...
from a ~40-page extraction — that decoupling is the entire reason the broker
is here.
...
called twice — real money. Mitigation: the cache in §5 turns the second call
