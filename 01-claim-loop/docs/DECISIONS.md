...
genuinely blank on the form is not an extraction failure, since no human reading it
...
with the `openai` SDK pointed at their `base_url`, with no new client library
...
- **The Batch API.** Anthropic's 50% async discount is a first-party feature
  ...
  so that discount was genuinely applicable, and I am choosing convenience over it.
...
**Why:** The CLI came first deliberately, because it makes it trivial to run two
...
credentials. **No service account key**, because the organisation enforces
